...
import { getAddress, SwitchChainError } from "@/lib/connect/viem";
...
export function authConnector(parameters: AuthParameters) {
  ...
  return createConnector<W3mFrameProvider, Properties>((config) => ({
    ...
    async connect(options = {}) {
      ...
      currentAccounts = accounts?.map((a) => a.address as Address) || [
        address as Address,
      ];
        ...
        this.provider = W3mFrameProviderSingleton.getInstance({
          ...
          onTimeout: () => {
            AlertController.open(
              ErrorUtil.ALERT_ERRORS.SOCIALS_TIMEOUT,
              "error"
            );
          },
        });
