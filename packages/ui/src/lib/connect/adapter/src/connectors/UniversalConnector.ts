...
export function walletConnect(
    ...
    type Properties = {
        ...
        getNamespaceChainsIds(): string[];
        getRequestedChainsIds(): Promise<string[]>;
        isChainsStale(): Promise<boolean>;
        onConnect(connectInfo: ProviderConnectInfo): void;
        onDisplayUri(uri: string): void;
        onSessionDelete(data: { topic: string }): void;
        setRequestedChainsIds(chains: string[]): void;
        requestedChainsStorageKey: `${string}.requestedChains`;
    };
    type StorageItem = {
        [_ in Properties["requestedChainsStorageKey"]]: string[];
    };
    ...
    return createConnector<Provider, Properties, StorageItem>((config) => ({
        ...
        async connect({...rest} = {}) {
            try {
                ...
                if (!provider.session || isChainsStale) {
                    ...
                    this.setRequestedChainsIds(caipNetworks.map((x) => String(x.id)));
                }
                ...
                return {accounts, networkId: currentChainId};
            } catch (error) {
                if (
        ...
        async getAccounts() {
            ...
            const accounts =
                accountsList?.map((account) => account.split(":")[2]) ?? [];
        ...
        async getProvider({networkId} = {}) {
            ...
            if (networkId && currentChainId !== networkId) {
                const storedCaipNetwork = StorageUtil.getStoredActiveCaipNetwork();

                if (
                    storedCaipNetwork &&
                    storedCaipNetwork.chainNamespace === ConstantsUtil.CHAIN.EVM
                ) {
                    await this.switchChain?.({networkId: storedCaipNetwork.id as string});
                }
            }
        ...
        async switchChain({addEthereumChainParameter, networkId}) {
            ...
            const chainToSwitch = caipNetworks.find((x) => x.id === networkId);
            ...
            try {
                ...
                this.setRequestedChainsIds([...requestedChains, networkId]);

                return {...chainToSwitch, id: chainToSwitch.id as string};
            } catch (err) {
        ...
        onChainChanged(chain) {
            config.emitter.emit("change", {networkId: chain});
        },
        ...
        async isChainsStale() {
            ...
            return !connectorChains.every((id) =>
