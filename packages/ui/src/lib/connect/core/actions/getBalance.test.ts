...
describe("getBalance", () => {
  test("default", async () => {
    await expect(getBalance(config, { address })).resolves
  ...
  test("parameters: chainId", async () => {
    await expect(getBalance(config, { address, chainId: chain.mainnet2.id }))
  ...
  test("parameters: unit", async () => {
    await expect(getBalance(config, { address, unit: "wei" })).resolves
      .toMatchInlineSnapshot(`
      {
        "decimals": 18,
