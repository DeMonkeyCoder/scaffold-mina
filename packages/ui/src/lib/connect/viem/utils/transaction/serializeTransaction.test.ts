...
import { toBlobs } from "../blob/toBlobs";
import { stringToHex } from "../index";
...
describe("eip4844", () => {
  ...
  test("networkID wrapper (blobVersionedHashes + sidecars)", () => {
  ...
  test("networkID wrapper (blobs + blobVersionedHashes + sidecars)", () => {
    const sidecars = toBlobSidecars({ data: stringToHex("abcd"), kzg });
    const blobVersionedHashes = sidecarsToVersionedHashes({ sidecars });
    const transaction = {
      ...baseEip4844,
  ...
  test("networkID wrapper (blobs + kzg)", () => {
    const transaction = {
      ...baseEip4844,
...
describe("github", () => {
  test("https://github.com/wevm/@/lib/connect/viem/issues/1433", () => {
    expect(
      serializeTransaction(
        {
          blockHash: null,
  ...
  test("https://github.com/wevm/@/lib/connect/viem/issues/1849", async () => {
    const tx = {
      blockHash:
  ...
  test("https://github.com/wevm/@/lib/connect/viem/issues/2394", async () => {
    const serialized = serializeTransaction(
      {
        chainId: 17000,
        gas: BigInt("0x52080"),
