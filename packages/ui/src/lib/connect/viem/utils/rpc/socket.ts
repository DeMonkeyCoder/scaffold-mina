import { SocketClosedError, TimeoutError } from "../../errors/request";
...
export async function getSocketRpcClient<socket extends {}>(
  ...
  >({
    ...
    fn: async () => {
      ...
      // Set up a cache for subscriptions (mina_subscribe).
      ...
      async function setup() {
        const result = await getSocket({
          ...
          onResponse(data) {
            const isSubscription = data.method === "mina_subscription";
      ...
      socketClient = {
        ...
        request({ body, onError, onResponse }) {
          ...
          const callback = (response: RpcResponse) => {
            ...
            if (body.method === "mina_unsubscribe")
