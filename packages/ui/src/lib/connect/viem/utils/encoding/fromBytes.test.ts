...
describe("converts bytes to number", () => {
  test("default", () => {
    ...
    expect(bytesToNumber(new Uint8Array([1, 164]))).toMatchInlineSnapshot(
      "420"
    );
  });
...
describe("converts bytes to bigint", () => {
  test("default", () => {
    expect(fromBytes(new Uint8Array([0]), "bigint")).toMatchInlineSnapshot(
      "0n"
    );
    expect(fromBytes(new Uint8Array([7]), "bigint")).toMatchInlineSnapshot(
      "7n"
    );
