...
import type { Quantity, RpcLog, RpcTransaction } from "./rpc";
...
test("default", async () => {
  ...
  const x = await request({ method: "mina_wagmi" });
  expectTypeOf<typeof x>().toEqualTypeOf<unknown>();

  request({ method: "mina_wagmi", params: undefined });
  request({ method: "mina_wagmi", params: [] });
  request({ method: "mina_wagmi", params: [{ foo: "bar" }] });

  expectTypeOf<DefaultParameters>().toEqualTypeOf<{
    method: string;
    params?: unknown;
  }>();
  expectTypeOf<ReturnType<DefaultRequestFn>>().toEqualTypeOf<
    Promise<unknown>
  >();
});
...
test("public methods", async () => {
  ...
  const x1 = await request({ method: "mina_blockNumber" });
  ...
  // @ts-expect-error
  request({ method: "mina_newFilter" });
  // @ts-expect-error
  request({ method: "mina_wagmi" });
...
test("wallet methods", async () => {
  ...
  const x1 = await request({ method: "mina_accounts" });
  ...
  // @ts-expect-error
  request({ method: "mina_sendTransaction" });
  // @ts-expect-error
  request({ method: "mina_wagmi" });
...
test("custom methods (strict)", async () => {
  type CustomSchema = [
    { Method: "mina_wagmi"; Parameters: [number]; ReturnType: string },
    { Method: "mina_viem"; Parameters?: never; ReturnType: number }
  ];
  ...
  const x1 = await request({ method: "mina_wagmi", params: [1] });
