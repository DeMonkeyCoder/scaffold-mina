...
function ContractMethod<T extends SmartContract>({
    ...
    const paramNames = useMemo(() => {
        const methodRegex = new RegExp(`async?\\s+${String(methodName)}\\s*\\(\\s*([a-zA-Z0-9_,\\s]*)\\s*\\)`, 'g');
        ...
        return paramString.split(',').map(param => param.trim()).filter(param => param !== '');
    }, [contract, methodName]);
    return <div className="mb-5 p-5 bg-purple-50 shadow-lg rounded-3xl border-stone-400 break-words">
        <div className="font-bold">
            {String(methodName)}
        </div>
        <div>
            {paramNames.map((paramName) => <div>
                <label>{paramName}</label>
                <br/>
                <input type="text" className="mt-2 py-1 px-3 border-gray-300 rounded-full border-2 w-full"/>
            </div>)}
        </div>
        <button
...
export default function DebugContracts() {
    ...
    const selectedContractMethods = useMemo(() => {
        if (selectedContract && chainContracts) {
            // @ts-ignore
            return chainContracts[selectedContract].contract["_methods"].map(m => m.methodName) as string[];
        }
    }, [chainContracts, selectedContract]);
    const selectedContractStates = useMemo(() => {
        if (selectedContract && chainContracts && selectedContractMethods !== undefined) {
            // @ts-ignore
            const notStateVariables = ["constructor", "init", ...selectedContractMethods]
    ...
    return <div className="pt-20 overflow-auto h-screen">
        <div className="px-5">
            {
            ...
            chainContracts && selectedContract && (
                <>
                    <div className="px-9 py-2">
                        ...
                        />)}
                        <div className="py-2 px-5 mb-2 bg-purple-50 rounded-3xl border-stone-400 w-100">Write</div>
                        {selectedContractMethods?.map((methodName) => <ContractMethod
                            key={methodName}
                            contract={chainContracts[selectedContract].contract}
                            // @ts-ignore
